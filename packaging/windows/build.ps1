...
foreach ($kind in @('console','gui')) {
    ...
    Run $csc @('/nologo','/optimize+','/platform:x64',$target,$define,('/out:' + $exe),('/win32icon:' + (Join-Path $PSScriptRoot 'jouzu.ico')),'/reference:System.Drawing.dll','/reference:System.Web.Extensions.dll','/reference:System.Windows.Forms.dll','/reference:System.Security.dll','/reference:System.Core.dll',(Join-Path $PSScriptRoot 'Jouzu.cs'),(Join-Path $PSScriptRoot 'Updates.cs'))
}
